...
class Admin extends AdminModule
{
    ...
    public function postSaveDetail()
    {
      ...
      if($_POST['kat'] == 'obat') {
        ...
        $cek_resep = $this->core->mysql('resep_obat')->where('no_rawat', $_POST['no_rawat'])->where('tgl_perawatan', $_POST['tgl_perawatan'])->oneArray();
        if(!$cek_resep) {
          $max_id = $this->core->mysql('resep_obat')->select(['no_resep' => 'ifnull(MAX(CONVERT(RIGHT(no_resep,4),signed)),0)'])->where('tgl_perawatan', '0000-00-00')->oneArray();
      ...
      if($_POST['kat'] == 'racikan') {
        $max_id = $this->core->mysql('resep_obat')->select(['no_resep' => 'ifnull(MAX(CONVERT(RIGHT(no_resep,4),signed)),0)'])->where('tgl_perawatan', $_POST['tgl_perawatan'])->oneArray();
        if(empty($max_id['no_resep'])) {
          $max_id['no_resep'] = '0000';
        }
        $_next_no_resep = sprintf('%04s', ($max_id['no_resep'] + 1));
    ...
    {
      $reg_periksa = $this->core->mysql('reg_periksa')->where('no_rawat', $_POST['no_rawat'])->oneArray();
      $depo = ($reg_periksa['kd_pj'] != 'A09') ? 'farmasi.deporalan' : 'farmasi.depoumum';
      $obat = $this->core->mysql('databarang')
        ->join('gudangbarang', 'gudangbarang.kode_brng=databarang.kode_brng')
        ->where('status', '1')
        ->where('stok','>', '0')
        ->where('gudangbarang.kd_bangsal', $this->settings->get($depo))
    ...
    public function getView($id, $page = 1)
    {
        $id = revertNorawat($id);
        $this->_addHeaderFiles();
        $reg_periksa = $this->core->mysql('reg_periksa')->where('no_rawat', $id)->oneArray();
        $pasien = $this->core->mysql('pasien')->where('no_rkm_medis', $reg_periksa['no_rkm_medis'])->oneArray();
        $personal_pasien = $this->core->mysql('personal_pasien')->where('no_rkm_medis', $reg_periksa['no_rkm_medis'])->oneArray();
        $count_ralan = $this->core->mysql('reg_periksa')->where('no_rkm_medis', $reg_periksa['no_rkm_medis'])->where('status_lanjut', 'Ralan')->count();
        $count_ranap = $this->core->mysql('reg_periksa')->where('no_rkm_medis', $reg_periksa['no_rkm_medis'])->where('status_lanjut', 'Ranap')->count();
        ...
        if (!empty($reg_periksa)) {
            ...
            $this->assign['soap'] = $this->core->mysql('pemeriksaan_ralan')->where('no_rawat', $id)->oneArray();
            $this->assign['metode_racik'] = $this->core->mysql('metode_racik')->toArray();
            $this->assign['diagnosa_pasien'] = $this->core->mysql('diagnosa_pasien')->join('penyakit', 'penyakit.kd_penyakit = diagnosa_pasien.kd_penyakit')->where('no_rawat', $id)->toArray();
            $this->assign['prosedur_pasien'] = $this->core->mysql('prosedur_pasien')->join('icd9', 'icd9.kode = prosedur_pasien.kode')->where('no_rawat', $id)->toArray();
            $this->assign['rawat_jl_dr'] = $this->core->mysql('rawat_jl_dr')->join('jns_perawatan', 'jns_perawatan.kd_jenis_prw = rawat_jl_dr.kd_jenis_prw')->where('no_rawat', $id)->toArray();
            $this->assign['rawat_jl_pr'] = $this->core->mysql('rawat_jl_pr')->join('jns_perawatan', 'jns_perawatan.kd_jenis_prw = rawat_jl_pr.kd_jenis_prw')->where('no_rawat', $id)->toArray();
            $this->assign['rawat_jl_drpr'] = $this->core->mysql('rawat_jl_drpr')->join('jns_perawatan', 'jns_perawatan.kd_jenis_prw = rawat_jl_drpr.kd_jenis_prw')->where('no_rawat', $id)->toArray();
            $this->assign['rawat_inap_dr'] = [];
            $this->assign['rawat_inap_pr'] = [];
            $this->assign['rawat_inap_drpr'] = [];
            if($this->db('mlite_settings')->where('module', 'rawat_inap')->oneArray()) {
              $this->assign['rawat_inap_dr'] = $this->core->mysql('rawat_inap_dr')->join('jns_perawatan', 'jns_perawatan.kd_jenis_prw = rawat_inap_dr.kd_jenis_prw')->where('no_rawat', $id)->toArray();
              $this->assign['rawat_inap_pr'] = $this->core->mysql('rawat_inap_pr')->join('jns_perawatan', 'jns_perawatan.kd_jenis_prw = rawat_inap_pr.kd_jenis_prw')->where('no_rawat', $id)->toArray();
              $this->assign['rawat_inap_drpr'] = $this->core->mysql('rawat_inap_drpr')->join('jns_perawatan', 'jns_perawatan.kd_jenis_prw = rawat_inap_drpr.kd_jenis_prw')->where('no_rawat', $id)->toArray();
            }
            $this->assign['catatan'] = $this->core->mysql('catatan_perawatan')->where('no_rawat', $id)->oneArray();
            $rows_permintaan_resep = $this->core->mysql('resep_obat')
                ->join('resep_dokter', 'resep_dokter.no_resep = resep_obat.no_resep')
                ->join('databarang', 'databarang.kode_brng = resep_dokter.kode_brng')
                ->where('no_rawat', $id)
                ->toArray();
            ...
            $this->assign['permintaan_resep_racikan'] = $this->core->mysql('resep_obat')
                ->join('resep_dokter_racikan', 'resep_dokter_racikan.no_resep = resep_obat.no_resep')
            ...
            $this->assign['permintaan_lab'] = $this->core->mysql('permintaan_lab')
                ->join('permintaan_pemeriksaan_lab', 'permintaan_pemeriksaan_lab.noorder = permintaan_lab.noorder')
                ->join('jns_perawatan_lab', 'jns_perawatan_lab.kd_jenis_prw = permintaan_pemeriksaan_lab.kd_jenis_prw')
                ->where('no_rawat', $id)
                ->toArray();
            $this->assign['permintaan_rad'] = $this->core->mysql('permintaan_radiologi')
                ->join('permintaan_pemeriksaan_radiologi', 'permintaan_pemeriksaan_radiologi.noorder = permintaan_radiologi.noorder')
            ...
            $this->assign['berkas_digital'] = $this->core->mysql('berkas_digital_perawatan')->where('no_rawat', $id)->toArray();

            $this->assign['manageURL'] = url([ADMIN, 'dokter_ralan', 'manage']);
            $totalRecords = $this->core->mysql('reg_periksa')
            ...
            foreach ($rows as &$row) {
                $pemeriksaan_ralan = $this->core->mysql('pemeriksaan_ralan')->where('no_rawat', $row['no_rawat'])->oneArray();
                if($row['status_lanjut'] == 'Ranap') {
                  $pemeriksaan_ralan = $this->core->mysql('pemeriksaan_ranap')->where('no_rawat', $row['no_rawat'])->oneArray();
                }
                $diagnosa_pasien = $this->core->mysql('diagnosa_pasien')->join('penyakit', 'penyakit.kd_penyakit = diagnosa_pasien.kd_penyakit')->where('no_rawat', $row['no_rawat'])->toArray();
                $prosedur_pasien = $this->core->mysql('prosedur_pasien')->join('icd9', 'icd9.kode = prosedur_pasien.kode')->where('no_rawat', $row['no_rawat'])->toArray();
                $rawat_jl_dr = $this->core->mysql('rawat_jl_dr')->join('jns_perawatan', 'jns_perawatan.kd_jenis_prw = rawat_jl_dr.kd_jenis_prw')->where('no_rawat', $row['no_rawat'])->toArray();
                $rawat_jl_pr = $this->core->mysql('rawat_jl_pr')->join('jns_perawatan', 'jns_perawatan.kd_jenis_prw = rawat_jl_pr.kd_jenis_prw')->where('no_rawat', $row['no_rawat'])->toArray();
                $rawat_jl_drpr = $this->core->mysql('rawat_jl_drpr')->join('jns_perawatan', 'jns_perawatan.kd_jenis_prw = rawat_jl_drpr.kd_jenis_prw')->where('no_rawat', $row['no_rawat'])->toArray();
                $rawat_inap_dr = [];
                $rawat_inap_pr = [];
                $rawat_inap_drpr = [];
                if($this->db('mlite_settings')->where('module', 'rawat_inap')->oneArray()) {
                  $rawat_inap_dr = $this->core->mysql('rawat_inap_dr')->join('jns_perawatan', 'jns_perawatan.kd_jenis_prw = rawat_inap_dr.kd_jenis_prw')->where('no_rawat', $row['no_rawat'])->toArray();
                  $rawat_inap_pr = $this->core->mysql('rawat_inap_pr')->join('jns_perawatan', 'jns_perawatan.kd_jenis_prw = rawat_inap_pr.kd_jenis_prw')->where('no_rawat', $row['no_rawat'])->toArray();
                  $rawat_inap_drpr = $this->core->mysql('rawat_inap_drpr')->join('jns_perawatan', 'jns_perawatan.kd_jenis_prw = rawat_inap_drpr.kd_jenis_prw')->where('no_rawat', $row['no_rawat'])->toArray();
                }
                $detail_pemberian_obat = $this->core->mysql('aturan_pakai')
                ...
                $catatan_perawatan = $this->core->mysql('catatan_perawatan')->where('no_rawat', $row['no_rawat'])->oneArray();
                $berkas_digital = $this->core->mysql('berkas_digital_perawatan')->where('no_rawat', $row['no_rawat'])->toArray();
    ...
    public function postSOAPSave($id = null)
    {
        ...
        if (!$errors) {
            ...
            $cek_no_rawat = $this->core->mysql('pemeriksaan_ralan')->where('no_rawat', revertNorawat($id))->oneArray();
            if(empty($cek_no_rawat['no_rawat'])) {

              if($check_db) {
                $query = $this->core->mysql('pemeriksaan_ralan')
                  ->save([
              ...
              for ($i = 0; $i < count($get_kd_jenis_prw); $i++) {
                  ...
                  $row = $this->core->mysql('jns_perawatan')->where('kd_jenis_prw', $kd_jenis_prw)->oneArray();
                  $query = $this->core->mysql('rawat_jl_dr')
                    ->save([
                      'no_rawat' => revertNorawat($id),
              ...
              $query = $this->core->mysql('catatan_perawatan')
                ->save([
                  'tanggal' => date('Y-m-d'),
                  'jam' => date('H:i:s'),
                  'no_rawat' => revertNorawat($id),
              ...
              $get_kd_jenis_prw = $_POST['kd_jenis_prw'];
              $this->core->mysql('rawat_jl_dr')->where('no_rawat', revertNorawat($id))->delete();
              for ($i = 0; $i < count($get_kd_jenis_prw); $i++) {
                  $kd_jenis_prw = $get_kd_jenis_prw[$i];
                  $row = $this->core->mysql('jns_perawatan')->where('kd_jenis_prw', $kd_jenis_prw)->oneArray();
                  $query = $this->core->mysql('rawat_jl_dr')
              ...
              if($this->core->mysql('catatan_perawatan')->where('no_rawat', revertNorawat($id))->oneArray()){
                $query = $this->core->mysql('catatan_perawatan')
                  ->where('no_rawat', revertNorawat($id))
    ...
    public function postRadiologiSave($id = null)
    {
        ...
        if (!$errors) {
            ...
            $query = $this->core->mysql('permintaan_radiologi')
              ->save([
                'noorder' => $no_order,
                'no_rawat' => revertNorawat($id),
            ...
            if ($query) {
                for ($i = 0; $i < count($_POST['kd_jenis_prw']); $i++) {
                  $query = $this->core->mysql('permintaan_pemeriksaan_radiologi')
                    ->save([
                      'noorder' => $no_order,
    ...
    public function postLaboratoriumSave($id = null)
    {
        ...
        if (!$errors) {
            ...
            $query = $this->core->mysql('permintaan_lab')
              ->save([
                'noorder' => $no_order,
                'no_rawat' => revertNorawat($id),
                'tgl_permintaan' => date('Y-m-d'),
            ...
            if ($query) {
                for ($i = 0; $i < count($_POST['kd_jenis_prw']); $i++) {
                  $query = $this->core->mysql('permintaan_pemeriksaan_lab')
                    ->save([
                      'noorder' => $no_order,
                      'kd_jenis_prw' => $_POST['kd_jenis_prw'][$i],
                      'stts_bayar' => 'Belum'
    ...
    public function postResepSave($id = null)
    {
        ...
        if (!$errors) {
            unset($_POST['save']);
            $query = $this->core->mysql('resep_obat')->where('no_rawat', revertNorawat($id))->where('status', 'ralan')->oneArray();
        ...
        if (!$errors) {
            ...
            if ($query) {
              ...
              $this->core->mysql('resep_dokter_racikan')
                ...
                for ($i = 0; $i < count($_POST['kode_brng']); $i++) {
                  $kapasitas = $this->core->mysql('databarang')->where('kode_brng', $_POST['kode_brng'][$i])->oneArray();
    ...
    {
        if ($pendaftaran = $this->core->mysql('resep_dokter')->where('no_resep', $no_resep)->where('kode_brng', $kode_brng)->oneArray()) {
            if ($this->core->mysql('resep_dokter')->where('no_resep', $no_resep)->where('kode_brng', $kode_brng)->delete()) {
    ...
    public function postKontrolSave($id = null)
    {
        ...
        if (!$errors) {
            ...
            $query = $this->core->mysql('skdp_bpjs')->save($_POST);
